...
Ingress 资源定义了上图中的各种路由规则，具体的规则定义请参考这一篇[文章](https://haozhangms.github.io/2020/08/14/Ingress/)。但要想让 Ingress 中的路由规则工作，我们需要运行一个 Ingress Controller，不同的 Kubernetes 环境提供的 Ingress Controller 各不相同，一般我们用官方维护的 [Nginx](https://kubernetes.github.io/ingress-nginx/)。
